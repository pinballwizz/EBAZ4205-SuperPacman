...
assign MODEL = 3'd5; // set game to Super PacMan
...
assign WAVE_DT = WAVE_DT_all[3:0];
...
assign CLK24M   = CLKS[0]; // 24 Mhz
assign CLK12M   = CLKS[1]; // 12 Mhz
assign CLKCPUx2 = CLKS[2]; // 6 Mhz

assign VCLK_x8  = MCLK;    // 48 Mhz
assign VCLK_x4  = CLKS[0]; // 24 Mhz
assign VCLK_x2  = CLKS[1]; // 12 Mhz
assign VCLK_x1  = CLKS[2]; // 6 Mhz
...
//-------------------------------------------------------------------------
dpram #(8,11) main_ram0( .clk_a(CPUCLKx2), .addr_a(cram_ad), .d_a(MCPU_DO), .q_a(mram_o0), .we_a(mram_w0), .clk_b(VCLKx4), .addr_b(vram_a), .d_b(8'b0), .q_b(vram_d[ 7:0]));
dpram #(8,11) main_ram1( .clk_a(CPUCLKx2), .addr_a(cram_ad), .d_a(MCPU_DO), .q_a(mram_o1), .we_a(mram_w1), .clk_b(VCLKx4), .addr_b(vram_a), .d_b(8'b0), .q_b(vram_d[15:8]));

dpram #(8,11) main_ram2( .clk_a(CPUCLKx2), .addr_a(mram_ad), .d_a(MCPU_DO), .q_a(mram_o2), .we_a(mram_w2), .clk_b(VCLKx4), .addr_b({ 4'b1111, spra_a }), .d_b(8'b0), .q_b(spra_d[ 7: 0]));
dpram #(8,11) main_ram3( .clk_a(CPUCLKx2), .addr_a(mram_ad), .d_a(MCPU_DO), .q_a(mram_o3), .we_a(mram_w3), .clk_b(VCLKx4), .addr_b({ 4'b1111, spra_a }), .d_b(8'b0), .q_b(spra_d[15: 8]));
dpram #(8,11) main_ram4( .clk_a(CPUCLKx2), .addr_a(mram_ad), .d_a(MCPU_DO), .q_a(mram_o4), .we_a(mram_w4), .clk_b(VCLKx4), .addr_b({ 4'b1111, spra_a }), .d_b(8'b0), .q_b(spra_d[23:16]));
//-------------------------------------------------------------------------
                                                                                                // (SCPU ADRS)
...
.clk_b(CPUCLKx2), 
.addr_b({1'b0, SCPU_ADRS[9:0]}), 
.d_b(SCPU_DO), 
.q_b(sram_o0), 
.we_b(sram_cs0 & SCPU_WE) 
);
